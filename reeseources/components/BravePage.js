...
import {source} from '../constants/BravePDF.js';

export default function BravePage({navigation: {goBack}}) {
  const onShare = async () => {
    try {
      if (Platform.OS !== 'ios') {
        const shareOptions = {
        ...
        const path = dirs.DocumentDir + '/' + 'Brave.pdf';
        console.log({path});
        RNFetchBlob.fs.writeFile(path, {source}, 'base64');
        Share.open({
          subject: 'PDF share',
          url: path,
        })
